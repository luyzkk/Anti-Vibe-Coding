...
describe('version bump 7.0.0 (init-refactor-v7 — 10-step pipeline, breaking)', () => {
  it('package.json has version 7.0.0', async () => {
    const pkg = await readJson('package.json')
    expect(pkg['version']).toBe(EXPECTED_VERSION)
  })

  it('.claude-plugin/plugin.json has version 7.0.0', async () => {
    const pkg = await readJson('.claude-plugin/plugin.json')
    expect(pkg['version']).toBe(EXPECTED_VERSION)
  })

  it('.claude-plugin/marketplace.json has version 7.0.0', async () => {
    const pkg = await readJson('.claude-plugin/marketplace.json')
    expect(pkg['version']).toBe(EXPECTED_VERSION)
  })

  it('plugin-manifest.json has version 7.0.0', async () => {
