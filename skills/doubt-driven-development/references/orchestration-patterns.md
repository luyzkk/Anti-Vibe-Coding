...
```
/review → code-smell-detector (with code-review-and-quality skill) → report
```
...
Plugin subagents go in `agents/` at the plugin root. This repo is a plugin (`.claude-plugin/plugin.json`), so `agents/code-smell-detector.md`, `agents/security-auditor.md`, and `agents/tdd-verifier.md` are auto-discovered when the plugin is enabled. No path configuration needed.
...
Don't redefine these. Layer your specialist personas (code-smell-detector, solid-auditor, security-auditor, tdd-verifier) on top of them.
...
three teammates using the existing agent types:
  ...
  - code-smell-detector — investigate race conditions and blocking calls
  ...
  - tdd-verifier        — propose tests that would distinguish between the
...
4. When `code-smell-detector` finds a `Promise.all` that should be sequential, it messages `security-auditor` to confirm the auth call isn't part of the race. `security-auditor` checks and replies — either confirming the race is the real issue or producing counter-evidence.
5. `tdd-verifier` proposes a focused integration test for whichever theory is winning, which the team uses to verify before declaring consensus.
...
```
/work → router-persona → "this needs a review" → code-smell-detector → router (paraphrases) → user
```
...
A `code-smell-detector` that internally invokes `security-auditor` when it sees auth code.
...
`/ship` calls a `pre-ship-coordinator` that calls a `quality-coordinator` that calls `code-smell-detector`.
