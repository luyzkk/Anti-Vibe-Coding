# Fase 01 — Skill `/anti-vibe-coding:iterate`

**Sizing:** ~1.5h  
**Arquivo a criar:** `f:\Projetos\Claude code\anti-vibe-coding\skills\iterate\SKILL.md`  
**Ação:** Criar arquivo novo (pasta `iterate/` não existe ainda)

## Contexto

Esta skill fecha o pipeline como "quarta pata" do mantra:

```
Ensinar → Planejar → Verificar → Iterar em produção
```

Cobre o ciclo pós-deploy completo em dois modos:
1. **Incident response** — bug relatado em produção
2. **Hardening pós-feature** — nenhum bug, mas hora de endurecer

Referencia as skills standalone do Plano 03:
- `/anti-vibe-coding:incident-response`
- `/anti-vibe-coding:defensive-patterns`
- `/anti-vibe-coding:centralize-config`

## Conteúdo completo a criar
...
Após criar o arquivo, confirmar:

- [ ] Arquivo criado em `anti-vibe-coding/skills/iterate/SKILL.md`
- [ ] Frontmatter válido (name, description, user-invocable, allowed-tools, argument-hint)
- [ ] Detecção de contexto CA-08 implementada (bloco de código)
- [ ] Detecção de modo implementada (bloco de código)
- [ ] Modo A (incident) com 7 steps em blocos de código
- [ ] Modo B (hardening) com 5 steps em blocos de código
- [ ] Regression test obrigatório antes do fix (Step 3 Modo A)
- [ ] Referências às skills do Plano 03 presentes
- [ ] Regras em bloco de código
- [ ] Total de linhas ≤ 200
- [ ] Commit no repositório `anti-vibe-coding/` (não no pai)

## Comando de commit

```bash
cd 'f:/Projetos/Claude code/anti-vibe-coding'
git add skills/iterate/SKILL.md
git commit -m "feat(iterate): add /iterate skill — quarta pata do pipeline pos-deploy"
```
