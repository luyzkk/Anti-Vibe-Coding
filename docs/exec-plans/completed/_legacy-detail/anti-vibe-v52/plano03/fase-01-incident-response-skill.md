# Fase 01 — Skill `/anti-vibe-coding:incident-response`

## Objetivo

Criar a skill `incident-response` que guia o dev por um fluxo disciplinado de resposta a incidentes pós-deploy. Princípio central: o regression test vem ANTES do fix — hardening não é fase separada, é hábito.
...
```
f:\Projetos\Claude code\anti-vibe-coding\skills\incident-response\SKILL.md
```

**Pré-condição:** O diretório `skills/incident-response/` não existe ainda. Criar antes de escrever o arquivo:

```bash
mkdir -p "f:/Projetos/Claude code/anti-vibe-coding/skills/incident-response"
```
...
```bash
# 1. Diretório criado
ls "f:/Projetos/Claude code/anti-vibe-coding/skills/incident-response/"
# Esperado: SKILL.md

# 2. Frontmatter válido
head -10 "f:/Projetos/Claude code/anti-vibe-coding/skills/incident-response/SKILL.md"
# Esperado: --- name: incident-response ... ---

# 3. Contagem de linhas (deve ser ≤200)
wc -l "f:/Projetos/Claude code/anti-vibe-coding/skills/incident-response/SKILL.md"
# Esperado: ≤200

# 4. $ARGUMENTS presente
grep '\$ARGUMENTS' "f:/Projetos/Claude code/anti-vibe-coding/skills/incident-response/SKILL.md"
...
git add skills/incident-response/SKILL.md
git commit -m "feat(skills): incident-response — fluxo disciplinado pós-deploy"
```

## Gotchas desta Fase

- O diretório `incident-response/` precisa ser criado antes de escrever o arquivo
- A instrução "regression test ANTES do fix" é o diferencial desta skill — não abreviar
- Blocos de código com backticks dentro do SKILL.md são instruções executáveis — não prose
- O `$ARGUMENTS` no final é obrigatório para que o Claude receba o contexto passado pelo usuário
- Manter ≤200 linhas — se o arquivo crescer, cortar prosa, não lógica
