...
4. Read the relevant active plan, or create one with `bun run new-plan "Title"`.
...
1. Create a new plan with `bun run new-plan "Title"`.
...
Each plan follows the 10-section harmonized template (D18): Goal, Scope, Assumptions, Risks, Execution Steps, Review Checklist, Validation Log, Compound Opportunity, Lessons Captured, Exit Criteria. `bun run new-plan` scaffolds it.
